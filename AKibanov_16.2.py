class Turtle:
    ...
    def count_moves(self, x2, y2):
        ...
        if x0 % self.s != 0 or y0 % self.s != 0:
            return None
# (!) Переделал, добавив условие выше ^^^
...
# (!) Проверяем ещё раз метод count_moves, но с другой черепашкой
t=Turtle(9, 9, 3)
print(t.count_moves(3, 3))
#Очевидно, что тут получится черепашке добраться до этой координаты
print('y=', t.go_up())
print('s=', t.evolve())
print(t.count_moves(3, 3))
#Из координаты (9,12) до (3,3) при шаге 4 не добраться по х, поэтому None
